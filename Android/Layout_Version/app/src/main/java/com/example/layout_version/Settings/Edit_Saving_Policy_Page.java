...
public class Edit_Saving_Policy_Page extends AppCompatActivity {
    public static ArrayList<Displayable_Setting> policy_list = null;
    public static Displayable_Setting current_policy = null;
...
//    }
//
//    public void set_entries_resolution(Spinner spinner_resolution, Drop_Down drop){
//
//        ArrayList<Resolution> camera_entries = drop.get_list();
...
//    }
    ...
    public void add_using_attributes(){
        ...
        linearLayout.removeAllViews();
        Attribute[] attributes = current_policy.get_edit_attributes();
        for(int i = 0; i < attributes.length; i++){
            ...
            if(attribute instanceof Drop_Down_Attribute)
                construct_drop_down_attribute(linearLayout, (Drop_Down_Attribute) attribute);
        }
    }
    ...
    public void construct_number_attribute(LinearLayout linearLayout, Number attribute){
        ...
        text_input.setText(attribute.value());
        ...
        text_input.addTextChangedListener(new TextWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                String text = s.toString();
                System.out.println("texted changed, setting, max time: " + text);
                System.out.println("max time: " + ((Saving_Policy)current_policy).get_max_time());
                attribute.set(new Integer(Integer.parseInt(text)));
                System.out.println("max time: " + ((Saving_Policy)current_policy).get_max_time());
    ...
    public void construct_drop_down_attribute(LinearLayout linearLayout, Drop_Down_Attribute attribute){
        ConstraintLayout camera_layout = (ConstraintLayout) LayoutInflater.from(this).inflate(R.layout.settings_drop_down_template, null);
        ImageView add_resolution_view = (ImageView) ((ConstraintLayout)camera_layout.getChildAt(0)).getChildAt(1);
        Spinner spinner_resolution = (Spinner) ((ConstraintLayout)camera_layout.getChildAt(0)).getChildAt(2);
        TextView textView = (TextView) ((ConstraintLayout)camera_layout.getChildAt(0)).getChildAt(0);
        textView.setText(attribute.value());
        View_Factory.set_entries(attribute.get_list(), spinner_resolution, Edit_Saving_Policy_Page.this);
